...
/** Atualiza um prospect existente (todos os campos, incluindo estágio). */
...
try {
    $stmt = $db->prepare('
        UPDATE prospects
        SET nome = ?, contato = ?, email = ?, telefone = ?, estagio = ?, descricao = ?, valor_estimado = ?, observacoes = ?, proximo_contato = ?, prioritario = ?
        WHERE id = ?
    ');
    $stmt->execute([$nome, $contato, $email, $telefone, $estagio, $descricao, $valorEstimado, $observacoes, $proximoContato, $prioritario, $id]);

    echo json_encode(['sucesso' => true, 'prospect_id' => $id]);
} catch (Throwable $e) {
    http_response_code(500);
    echo json_encode(['erro' => 'Falha ao atualizar prospect', 'detalhe' => $e->getMessage()]);
}
